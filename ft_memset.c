...
void test_memset(const char *test_name, size_t size, int fill_char, const char *initial_data)
{
    ...
    printf("Size: %zu, Fill char: %d ('%c'), Initial: \"%s\"\n", 
           size, fill_char, (fill_char >= 32 && fill_char <= 126) ? fill_char : '?', 
           initial_data ? initial_data : "random");
    
    // Allocate test buffers
    char *test1 = malloc(size + 1);
    char *test2 = malloc(size + 1);
    ...
    void *result1 = ft_memset(test1, fill_char, size);
    void *result2 = memset(test2, fill_char, size);
    
    printf("ft_memset:  \"%.*s\"\n", (int)size, test1);
    printf("memset:     \"%.*s\"\n", (int)size, test2);
    
    // Check if results match
    int content_match = (memcmp(test1, test2, size) == 0);
    int return_match = (result1 == test1) && (result2 == test2);
...
int main(void)
{
    ...
    test_memset("Fill with large int", 6, 300, "hello!");  // Tests truncation to unsigned char
